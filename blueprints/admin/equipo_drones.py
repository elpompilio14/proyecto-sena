...
def equipo_drones_crear():
    ...
    with obtener_conexion() as conexion:
        with conexion.cursor() as cur:
            cur.execute(
                'INSERT INTO equipo_drones (nombre, rol, salon, foto_url, orden, nivel) VALUES (%s, %s, %s, %s, %s, %s)',
...
def equipo_drones_editar(id):
    ...
    with obtener_conexion() as conexion:
        with conexion.cursor() as cur:
            cur.execute(
                'UPDATE equipo_drones SET nombre = %s, rol = %s, salon = %s, foto_url = %s, orden = %s, nivel = %s WHERE id = %s',
