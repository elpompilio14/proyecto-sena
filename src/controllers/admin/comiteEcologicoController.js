...
exports.index = async (req, res) => {
    ...
    res.render('admin/comite-ecologico', {
        titulo: 'Admin · Comité Ecológico',
        miembros: miembros.rows,
        infoTexto: info.rows[0] ? info.rows[0].comite_ecologico_texto : '',
        infoPortada: info.rows[0] ? info.rows[0].comite_ecologico_portada_url : null,
    });
};
...
exports.actualizarInfo = async (req, res) => {
    ...
    await pool.query(
        'UPDATE institucion_info SET comite_ecologico_texto = $1, comite_ecologico_portada_url = $2 WHERE id = (SELECT id FROM institucion_info ORDER BY id LIMIT 1)',
        [texto || null, portada_url]
    );
    res.redirect('/admin/comite-ecologico');
};
...
exports.crear = async (req, res) => {
    ...
    await pool.query(
        'INSERT INTO comite_ecologico (nombre, rol, salon, foto_url, orden, nivel) VALUES ($1, $2, $3, $4, $5, $6)',
...
exports.eliminar = async (req, res) => {
    await pool.query('DELETE FROM comite_ecologico WHERE id = $1', [req.params.id]);
    res.redirect('/admin/comite-ecologico');
};

exports.toggleVisible = async (req, res) => {
    const r = await pool.query('UPDATE comite_ecologico SET visible = NOT visible WHERE id = $1 RETURNING visible', [req.params.id]);
    res.json({ visible: r.rows[0].visible });
};

exports.editarForm = async (req, res) => {
    const miembro = await pool.query('SELECT * FROM comite_ecologico WHERE id = $1', [req.params.id]);
    if (miembro.rows.length === 0) {
        return res.status(404).render('404', { titulo: 'No encontrado' });
    }
    res.render('admin/comite-ecologico-editar', { titulo: 'Admin · Editar miembro', miembro: miembro.rows[0] });
};
...
exports.editar = async (req, res) => {
    ...
    await pool.query(
        'UPDATE comite_ecologico SET nombre = $1, rol = $2, salon = $3, foto_url = $4, orden = $5, nivel = $6 WHERE id = $7',
